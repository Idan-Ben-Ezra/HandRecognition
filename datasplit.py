...
def split_dataset_into_3(path_to_dataset, train_ratio, valid_ratio):
    ...
    for i, sub_dir in enumerate(sub_dirs):
        ...
        for item_idx in tqdm(list(range(round(sub_dir_item_cnt[i] * train_ratio)))):
        ...
        for item_idx in tqdm(list(range(round(sub_dir_item_cnt[i] * train_ratio) + 1,
        ...
        for item_idx in tqdm(list(range(round(sub_dir_item_cnt[i] * (train_ratio + valid_ratio)) + 1, sub_dir_item_cnt[i]))):
...
if __name__ == "__main__":
    split_dataset_into_3("D:\magshimim\project\dataset\Dataset", 0.85, 0.1)